...
public class BaseFriendshipManager<Data extends BaseData<?>, FriendData extends BaseFriendData> {
    ...
    public Data addFriend(Data data) {
        ...
        String statement = "REPLACE INTO " + BaseConfig.TABLE_FRIENDS_LIST + " (" + FIELD_OLDER_USER_ID + ", " +
        ...
        if (wasSuccessful) {
...

            {
                ...
                BaseNotificationManager.sendNotification(receiverID, appName, tag, title, message);
            }
        }
    ...
    public Data removeFriend(Data data) {
        ...
        String statement = "DELETE FROM " + BaseConfig.TABLE_FRIENDS_LIST + " WHERE " +
    ...
    public Data block(Data data) {
        ...
        String statement = "REPLACE INTO " + BaseConfig.TABLE_FRIENDS_LIST + " (" +
    ...
    private static <FriendData extends BaseFriendData> List<FriendData> getActivitySortedFriendsList(int userID, String appName, String language) {
            ...
            for (Friendship friendship : RELATIONS) {
                ...
                if (friendship.olderUserID == userID || friendship.newerUserID == userID) {
                    ...
                    if (friendshipState == STATE_PENDING) {
                        ...
                        FriendData friend = (FriendData) BaseClasses.construct(BaseClasses.getInstance().friendClass);
    ...
    public static int getOnlineStatus(int userID) {
        ...
        long lastAccessTime = BaseResources.getInstance().requestManager.getLastAccessTime(userID);

        long currentTime = System.currentTimeMillis();

        long diff = currentTime - lastAccessTime;

        if (diff < BaseConfig.TWO_MINUTES) {

            return ONLINE_STATUS_ONLINE;
        }

        if (diff < BaseConfig.FIVE_MINUTES) {
    ...
    public static void notifyPlayerIsOnline(int userID, String appName) {
        ...
        if (onlineFriendIDs.size() > 0) {
            ...
            if (onlineFriendIDs.size() > 1) {
                ...
                BaseNotificationManager.sendMessage(userID, appName, message);
        ...
        for (int friendID : onlineFriendIDs) {
            ...
            BaseNotificationManager.sendMessage(friendID, appName, message);
        }
    }
